<template>
    <div>
        <!-- Login with code -->
        <CodeEntryScreen @login="onLogin" :initial="urlCode" gameName="Test Factory" icon="fas fa-cog"
            submitText="Start Shift!" v-if="currentPage === 'login'" 
        />
        <!-- Lobby, wait for players to join -->
        <div class="page-lobby text-center jumbotron" v-if="currentPage === 'lobby'">
            <h1>Players Joined:</h1>
            <button @click="startGame" class="btn btn-success" v-if="showStart">Start Game</button>
            <hr />
            <div class="card text-white bg-primary mb-3 d-inline-block" v-for="(player, i) in players" :key="i">
                <h4>{{ player.name }}</h4>
            </div>
        </div>
        <!-- Question page -->
        <div class="page-question text-center jumbotron" v-if="currentPage === 'question' && roomData">
            <h1>Question:</h1>
            <p class="lead">{{ roomData.question }}</p>

            <hr />

            <input type="text" class="form-control" v-model="currentAnswer" placeholder="Type your answer..." />
            <button @click="submitAnswer" class="btn btn-info">Submit Your Answer</button>

            <hr />

            <div>
                <h3>Ready Players</h3>
                <div class="card text-white bg-primary mb-3 d-inline-block" v-for="(player, i) in readyPlayers"
                    :key="i">
                    <h4>{{ player.name }}</h4>
                </div>
            </div>
        ...
        <div class="page-voting text-center jumbotron" v-if="currentPage === 'voting'">
            <h1>Vote for an answer:</h1>
            <div class="card text-white bg-primary mb-3 d-inline-block" v-for="(answer, i) in answers"
                :key="i">
        ...
        <div class="page-lb text-center jumbotron" v-if="currentPage === 'lb'">
            <h1>Top Players</h1>
            <div class="card text-white bg-primary mb-3 d-inline-block" 
                v-for="(player, i) in sortedPlayers" :key="i">
                <div class="card-body">
            ...
            <div class="card text-white bg-primary mb-3 d-inline-block" 
                ...
                <div class="card-body">
                    <p class="lead">
                        Answer: <b>{{ answer.answer }}</b><br>
                        Correct: <b>{{ answer.type === "real" ? "Yes!" : "No." }}</b><br>
        ...
        <div class="page-thx text-center jumbotron" v-if="currentPage === 'thx'">
...
export default Vue.extend({
    ...
    computed: {
        players(): IPlayer[] {
            return Object.keys(this.roomData?.readOnlyplayers || {}).map(k => {
    ...
    methods: {
        initSocket() {
            ...
            //Update roomState
            socket.on("updateRoomState", (newRoomData: IRoomData, subEvent: string) => {
        ...
        submitAnswer() {
            let socket = this.socket;
            socket.emit("submitAnswer", this.code, store.playerID, this.currentAnswer)
        },
