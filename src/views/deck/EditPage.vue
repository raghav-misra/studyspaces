<template>
    <section class="text-center">
        <div class="jumbotron">
            <h1>Editing <code><input class="title-input" v-model="title" /></code></h1>
            <hr />
            <div>
                <EditorField v-for="card in cards" :id="card.id" :key="card.id"
                    @update="update"
                    @remove="remove"
                />
...
<script lang="ts">
import Vue from "vue";
import { APIGetDeck, APICreateDeck, APIEditDeck } from "@/services/api";
import EditorField from "@/components/EditorField.vue";
import store from "@/store";
import { IUICard, ICardField } from "@/types";
...
export default Vue.extend({
    ...
    computed: {
        newDeck() { return this.id === "new-deck"; }
    },
    ...
    methods: {
        ...
        loadDeck(deckObject: Record<string, any>) {
        ...
        async saveDeck() {
            // Create New Deck:
            if (this.newDeck) {
                const creationSuccess = await APICreateDeck(store.token as string, {
                    title: this.title,
                    cards: this.cards,
                    img: this.img,
                    id: this.id
...
<style scoped>
h1 > code:first-of-type { position: relative; bottom: 2.5px; }

.title-input { color: #e83e8c; font-weight: bold; }
</style>