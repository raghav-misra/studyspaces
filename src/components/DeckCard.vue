<template>
    <div :class="`card d-${hidden ? 'none' : 'inline-flex'}`">
        <div v-if="type === 'preview' && deckObject">
            <img class="card-img-top" alt="Deck Image"
...
export default Vue.extend({
    created() {
        if (this.deck) APIGetDeck(this.deck).then(this.loadDeck);
...
.create-card:hover { background: var(--success); border-color: var(--success); }
